...
class App extends Component {
  ...
  newQuote() {
    //const name = quotes.from;
    let number = Math.floor(Math.random()*325);
    let quote = quotes[number].text;
    this.setState({
      text: quote
    })
    let author = quotes[number].from;
    this.setState({
      from: author
    })
  }
  ...
  render() {
    return (
        <div id="quote-box">
          <div id="text">
             <Text quote={this.state.text} />
          </div>
          <div id="author">
             <Text author={this.state.from} />
          </div>
          <div >
             <Button newHandleQuote={this.newQuote} quote={this.state.text} author={this.state.from}/>
          </div>
        </div>
    );
  }
...
class Button extends Component {
  render() {
   const dummi = "Today's Quote!  "
    return (
      <div id="quote-button">
      <button  onClick={this.props.newHandleQuote} id="new-quote">New</button>
      <a href={`https://twitter.com/intent/tweet/?text=${dummi}${this.props.quote} - ${this.props.author}`} ><button id="tweet-quote">Tweet me!</button></a>
      </div>
    )
  }
}
